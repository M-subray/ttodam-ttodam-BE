  ...
  public PostDetailDto getPostDetail(Long postId) {
    ...
    if (!isAuthor) {
      ...
      if (requestList != null && !requestList.isEmpty()) {
        for (ParticipationEntity request : requestList) {
          if (request.getRequestUser().getId().equals(userId)) {
            // 요청자인 경우 상태 반환
            if (request.getParticipationStatus() == ParticipationStatus.ACCEPT) {
              loginUserRequestStatus = "ACCEPT";
            } else if (request.getParticipationStatus() == ParticipationStatus.REFUSE) {
